...
int main4()
{
	...
	if (pid == 0)
	{
		...
		MPI_Recv(&sum, 50, MPI_INT, 2, 52, MPI_COMM_WORLD, &sta);
	...
	{
		num2 = 2;
		MPI_Send(&num2, 1, MPI_INT, 2, 51, MPI_COMM_WORLD);
	}
	...
	if (pid == 2)
	{
		...
		MPI_Recv(&recv2, 50, MPI_INT, 1, 51, MPI_COMM_WORLD, &sta);
		printf("\n My proccesoor id is %d and I'm receiving %d", pid, recv2);
		sum = recv1, recv2;
		MPI_Send(&sum, 1, MPI_INT, 0, 52, MPI_COMM_WORLD);
	}
